/* ============================================================
   ...
   Custom cursor, scroll animations, counters, curtain reveal,
   mobile menu
   ============================================================ */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── GUARANTEE STRIP REVEAL ── */
  const gstrip = document.querySelector('.guarantee-strip');
  if (gstrip) observer.observe(gstrip);
  ...
  if (heroLines) {
    window.addEventListener('scroll', () => {
      heroLines.style.transform = `translateY(${window.scrollY * 0.3}px)`;
    }, { passive: true });
  }
  ...
  document.querySelectorAll('.product-card').forEach(card => {
    card.addEventListener('click', e => {
      ...
      const name = card.querySelector('.product-name')?.textContent?.replace(/\s+/g, ' ').trim() || 'motor';
  ...
  /* ── IMAGE LOADING — curtain handles via CSS already
